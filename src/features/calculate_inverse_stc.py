...
from config_inverse_stc import study_path, subjects_dir, spacing, meg_dir, mindist
...
###################################################
# CODE ADAPTED FROM 
# https://github.com/BabaSanfour/MFRS/blob/main/meg_data_processing/04-calculate_inverse_stc.py 
###################################################
...
def setup_filenames(subject: str, spacing: str, task: str, meg_picks: str = True) -> dict:
    ...
    if meg_picks==True:
        meg_picks="mag"
    filenames = {
        'trans': os.path.join(RESULT_PATH, 'mri', task, f'{subject}/{subject}-trans.fif'),
        ...
        'fwd': os.path.join(RESULT_PATH, 'mri', task, subject, 'src', f'{subject}-{meg_picks}-eeg-{spacing}-fwd.fif'),
        'cov': os.path.join(RESULT_PATH, 'mri', task, subject, 'src', f'{subject}-cov.fif'),
        'inv': os.path.join(RESULT_PATH, 'mri', task, subject,'src', f'{subject}-{meg_picks}-{spacing}-inv.fif'),
        'stc': os.path.join(RESULT_PATH, 'mri', task, subject, 'src'),
        'mrph': os.path.join(RESULT_PATH, 'mri', task, subject, 'morph'),
        'avg_tcs': os.path.join(RESULT_PATH, 'mri', task, subject, 'src', f'{subject}-{meg_picks}-ROI-avg-time-courses.pkl'),
        'raw_tcs': os.path.join(RESULT_PATH, 'mri', task, subject, 'src', f'{subject}-{meg_picks}-ROI-raw-time-courses.pkl'),
        'trans_avg_tcs': os.path.join(RESULT_PATH, 'mri', task, subject, 'src', f'{subject}-{meg_picks}-ROI-trans-avg-time-courses.pkl'),
        'trans_raw_tcs': os.path.join(RESULT_PATH, 'mri', task, subject, 'src', f'{subject}-{meg_picks}-ROI-trans-raw-time-courses.pkl')
    }
