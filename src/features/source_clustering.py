...
def prepare_source_erp(task, cond1, cond2, save=True) :
    ...
    if save == True :
        ...
        filename_contrast, _ = get_bids_file(RESULT_PATH, task=task, stage='erp-source-contrast', condition=conditions)
        filename_cond1, _ = get_bids_file(RESULT_PATH, task=task, stage='erp-source-contrast', condition=cond1_name)
        filename_cond2, _ = get_bids_file(RESULT_PATH, task=task, stage='erp-source-contrast', condition=cond2_name)
...
def cluster_source(task, contrast, conditions) :

    # Find adjacency
    subj = '01'
    file_src = 'sub-{}-mag-eeg-oct6-fwd.fif'.format(subj)
    path_src = os.path.join(MRI_PATH, f'sub-{subj}', 'src', file_src)
...
if __name__ == "__main__" :
    ...
    contrast, conditions = prepare_source_erp(task, cond1, cond2, save)
    cluster_source(task, contrast, conditions)